<script lang="ts">
	import { goto } from '$app/navigation';
	import DataTable from '$lib/components/shared/DataTable.svelte';
	import { formatCurrency } from '$lib/utils';
	import type { Product } from '$lib/api/types';
	import type { PageData } from './$types';

	let { data }: { data: PageData } = $props();

	const columns = [
		{ key: 'id', header: 'ID', class: 'w-16' },
		{ key: 'name', header: 'Name' },
		{ key: 'category', header: 'Category' },
		{
			key: 'price',
			header: 'Price',
			render: (item: Product) => formatCurrency(Number(item.price || 0))
		},
		{
			key: 'cost',
			header: 'Cost',
			render: (item: Product) => formatCurrency(Number(item.cost || 0))
		},
		{
			key: 'inventory',
			header: 'Stock',
			render: (item: Product) => item.inventory?.toString() || '0'
		}
	];

	function handleRowClick(product: Product) {
		goto(`/admin/products/${product.id}`);
	}
</script>

<svelte:head>
	<title>Products - Admin - Coffee Shop</title>
</svelte:head>

<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div>
			<h2 class="text-2xl font-bold text-coffee-900">Products</h2>
			<p class="text-coffee-600">Manage your product catalog</p>
		</div>
		<a
			href="/admin/products/new"
			class="flex items-center gap-2 rounded-md bg-coffee-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-coffee-700"
		>
			<span class="text-lg">+</span>
			Add Product
		</a>
	</div>

	{#if data.error}
		<div class="rounded-md bg-red-50 p-4 text-sm text-red-700">
			{data.error}
		</div>
	{/if}

	<DataTable
		data={data.products || []}
		{columns}
		onRowClick={handleRowClick}
		emptyMessage="No products found. Add your first product to get started."
	/>
</div>
